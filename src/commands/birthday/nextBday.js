...
module.exports = {
    ...
    callback: async (client, interaction) => {
        ...
        try {
            ...
            const displayName = await interaction.guild.members.fetch(users[index].userId).displayName;
            if (isBday) {
                const bdayName = await interaction.guild.members.fetch(users[index-1].userId).displayName;
